...
class RepositoryModelResolver implements ModelResolver {
	...
	private static final Logger slf4jLogger = LoggerFactory.getLogger(RepositoryModelResolver.class);
	...
	public ModelSource resolveModel(String groupId, String artifactId, String versionId) throws UnresolvableModelException {
		...
		if (!pom.exists()) {
			slf4jLogger.info("Not Found POM: " + pom);
			try {
				download(pom);
			} catch (IOException e) {
			...
			slf4jLogger.info("Found POM: " + pom);
		}
	...
	public void download(File localRepoFile) throws IOException {
		for (Repository repository1 : repositories) {
			...
			slf4jLogger.info("Downloading " + url);
			...
			try {
				...
				slf4jLogger.info("Failed to download " + url);
			}
		}
